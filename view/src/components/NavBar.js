                ...
                <div className="flex items-center md:order-2 relative">
                    ...
                    {login ? <div className={`absolute top-2 left-10 z-50 my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700 dark:divide-gray-600 ${clicked ? 'visible' : 'hidden'}`} id="user-dropdown">
                        ...
                        <ul className="py-2" aria-labelledby="user-menu-button">
                            ...
                            <li>
                                <button to="/login" onClick={handleLogout} className={({ isActive, isPending }) => isPending ? "pending" : isActive ? "active" : "block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white"} style={({isActive, isPending}) => {return {color: isActive ? 'wheat' : ''}}}>Sign Out</button>
                            </li>
                        </ul>
                    </div> : <></>}
